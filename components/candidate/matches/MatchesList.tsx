...
import MatchesCVBar from "./MatchesCVBar";
...
export default function MatchesList({ jobs, cvAnalysis }: MatchesListProps) {
  ...
  return (
    <div>
      ...
      {/* CV info bar */}
      <MatchesCVBar cvAnalysis={cvAnalysis} />
      ...
      {filtered.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          <AnimatePresence>
            {filtered.map((job, i) => (
              <MatchesCard key={job.id} job={job} index={i} />
            ))}
          </AnimatePresence>
        </div>
      )}

...
// app/dashboard/candidate/_components/MiniCalendar.tsx
...
import { formatDate, formatTime, DAYS_ID, MONTHS_ID } from "./helpers";
...
export function MiniCalendar({ interviews }: { interviews: Interview[] }) {
  const today = new Date();
  const [viewYear, setViewYear] = useState(today.getFullYear());
  const [viewMonth, setViewMonth] = useState(today.getMonth());
  const [selectedDate, setSelectedDate] = useState<string | null>(null);

  // Map interview dates → interview list
  ...
  // Auto-select nearest upcoming interview date
  useEffect(() => {
  ...
  const selectedInterviews = selectedDate
    ? interviewDateMap.get(selectedDate) || []
    : [];
    ...
    <div className="bg-[#0a0f0c] border border-cyan-500/20 rounded-[18px] overflow-hidden mb-4">
          ...
          {cells.map((day, idx) => {
            ...
            return (
              <button
                key={key}
                onClick={() =>
                  hasInterview && setSelectedDate(isSelected ? null : key)
                }
                ...
                {hasInterview && !isSelected && (
                  <div className="absolute bottom-[3px] flex gap-[2px]">
                    {isOnline && (
                      <div className="w-[3px] h-[3px] rounded-full bg-violet-400" />
                    )}
                    {isOnsite && (
                ...
                {selectedInterviews.map((iv) => {
                  ...
                  return (
                    <div
                      ...
                      <div
                        className={`w-7 h-7 rounded-[7px] flex items-center justify-center flex-shrink-0 ${isOnline ? "bg-violet-500/15 text-violet-400" : "bg-orange-500/15 text-orange-400"}`}>
      ...
      {upcomingAll.length > 0 && !selectedDate && (
        <div className="px-4 pb-4">
          <div className="text-[0.68rem] text-[#7a9585] font-bold uppercase tracking-wider mb-2">
            Akan datang
          </div>
          <div className="space-y-2">
      ...
      {upcomingAll.length === 0 && (
        <div className="px-4 pb-4 text-center">
          <div className="text-[0.72rem] text-[#7a9585] py-3">

...
// ── Main Card ─────────────────────────────────────────────────────────────────
export function AiInsightCard({ cv }: { cv: CvAnalysis | null }) {
  ...
  return (
    <div className="bg-[#0a0f0c] border border-emerald-500/20 rounded-[18px] overflow-hidden mb-5">
      ...
      {/* Expandable insights */}
      {(cv.strengths?.length > 0 || cv.improvements?.length > 0) && (
        <>
          <button
                ...
                <div className="px-5 pb-5 grid grid-cols-2 gap-4">
                  {cv.strengths?.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-[6px]">
                        {cv.strengths.slice(0, 4).map((s, i) => (
                          <div
                            key={i}
                  ...
                  {cv.improvements?.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-[6px]">
                        {cv.improvements.slice(0, 4).map((s, i) => (
                          <div
                            key={i}
                            className="flex items-start gap-2 text-[0.73rem] text-[#a0b5aa]">

...
// untuk animasi fade-in saat halaman pertama dimuat.
...
export default function CompanyHero({ companyCount, totalJobs }: CompanyHeroProps) {
  return (
    ...
    >
      ...
      >
        {/* Badge */}
        <div className="inline-flex items-center gap-[6px] bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 px-[13px] py-[4px] rounded-full text-[0.68rem] font-semibold tracking-[0.1em] uppercase mb-[16px]">
          <span className="animate-pulse text-[8px]">●</span> Direktori Perusahaan
        </div>
        ...
        <p className="text-[#5d7a6a] text-[0.88rem] leading-[1.7] mb-7 max-w-[480px] mx-auto">
          Temukan perusahaan impian kamu, pelajari kultur kerja mereka, dan lamar ke
          posisi yang tersedia sekarang.
        </p>

        {/* Stats — data sudah tersedia dari server saat render */}
        <div className="inline-flex items-center gap-5 bg-[#0f1612] border border-white/[0.07] rounded-full px-5 py-[9px] text-[0.72rem]">
          <span className="flex items-center gap-[5px] text-[#5d7a6a]">
            ...
            <span className="text-[#e8f0ec] font-semibold">{companyCount}</span> perusahaan
          </span>
          <span className="w-px h-3 bg-white/[0.07]" />
          <span className="flex items-center gap-[5px] text-[#5d7a6a]">
            <Briefcase size={12} className="text-emerald-400" />
            <span className="text-[#e8f0ec] font-semibold">{totalJobs}</span> lowongan aktif
          </span>
          <span className="w-px h-3 bg-white/[0.07]" />
          <span className="flex items-center gap-[5px] text-[#5d7a6a]">
            <Globe size={12} className="text-emerald-400" />
            <span className="text-[#e8f0ec] font-semibold">Remote</span> tersedia
          </span>
        </div>
      </motion.div>
    </section>

...
export function AvatarCard({
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addToast("success", "Foto profil berhasil diperbarui!");
    } catch (err: unknown) {
  ...
  return (
    <div className="rounded-[16px] border border-emerald-500/15 p-6 flex flex-col items-center gap-4 bg-[#0f1612]">
      {/* Avatar dengan hover overlay untuk upload */}
      <div className="relative group">
        {avatarSrc ? (
          ...
          <div
            aria-label={`Inisial profil ${displayName}`}
        ...
        {role === "candidate" && (
          <>
            <button
              ...
              disabled={uploading}
              title={
                uploading ? "Sedang mengupload foto…" : "Ganti foto profil"
              }
              aria-label={
                uploading ? "Sedang mengupload foto…" : "Ganti foto profil"
              }
      ...
      {/* Mini stats — data sudah dari server, tidak perlu fetch ulang */}
      {role === "candidate" && (

...
// ─────────────────────────────────────────────────────────────────────────────
// AI INSIGHT PANEL — CSR (derived dari kandidat state, client-side computation)
// Route: @/components/hr/dashboard/AIInsightPanel.tsx
// ─────────────────────────────────────────────────────────────────────────────
...
import { CandidateExtended } from "./types";
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          {[
            ...
            >
              <div className="text-[0.95rem] font-black" style={{ color: m.color }}>
                {m.val}
              </div>
              <div className="text-[0.58rem] text-[#7a9585] mt-[1px]">{m.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {insights.length > 0 && (
        <div
          ...
          style={{ gridTemplateColumns: `repeat(${insights.length},minmax(0,1fr))` }}
        >
          {insights.map((ins, i) => {
            const InsIcon = ICON_MAP[ins.iconName as keyof typeof ICON_MAP] ?? Target;
            return (
              <div
                key={i}

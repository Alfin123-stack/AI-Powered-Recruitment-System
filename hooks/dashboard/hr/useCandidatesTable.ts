...
import type { SortKey, SortDir, StatusFilter, DateFilter, CandidateExtended } from "@/types/candidates";
...
export function useCandidatesTable(
  ...
  const [activeJob, setActiveJob] = useState<string>(initialJob ?? searchParams.get("job") ?? "all");
  ...
  const handleSelectJob = (job: string) => {
    ...
    else router.replace(`/dashboard/hr/candidates?job=${encodeURIComponent(job)}`);
  };

  const handleSearchChange = (value: string) => { setSearch(value); setPage(1); };
  const handleStatusChange = (status: StatusFilter) => { setActiveStatus(status); setPage(1); };

  const scoped = activeJob === "all" ? candidates : candidates.filter((c) => c.job === activeJob);
